...
if nargin>2        Mode=arg3;else        Mode=1;end;
%%%%% Load data 
if isstr(arg1),  % if string, assume arg1 is a Filename and SaO2 is at channel 16        if nargin<2, CH=16; else CH=arg2; end;        [sa,HDR] = sload(arg1,CH);
        Fs = HDR.SampleRate;           if isempty([findstr(HDR.PhysDim,'%'),findstr(lower(HDR.PhysDim),'percent')]),     %check Physical Dimension                  if ~isfield(HDR.FILE,'stderr'), HDR.FILE.stderr=2; end; % define error output                fprintf(HDR.FILE.stderr,'Warning DESATUR: Invalid Physical dimension\n');        end;        HDR.AS.SaO2.Fs=Fs;        HDR.AS.SaO2.Chan=CH;
