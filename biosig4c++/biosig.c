/*
    ...
    $Id: biosig.c,v 1.279 2009-01-19 15:36:14 schloegl Exp $
 ...
 */
	...
	hdr->data.block = (biosig_data_type*)malloc(0); 
      	hdr->T0 = (gdf_time)0; 
      	hdr->tzmin = 0; 
      	hdr->ID.Equipment = *(uint64_t*) & "b4c_0.79";
...
//			if (ptr_str != NULL) { 
		...
		if (EventChannel) {
			...
			// TODO: remove header for event channel	
			uint16_t k17=0; 		
...
//		B4C_ERRNUM = B4C_FORMAT_UNSUPPORTED; 
//		B4C_ERRMSG = "alpha format not supported";
		...
		// alpha.alp  digin  digvidtc  eog  marker  measure  mkdef  montage  rawdata  rawhead  report.txt  r_info  sleep
...
if (VERBOSE_LEVEL>7) fprintf(stdout,"%i- %s = %s\n",STATUS,t,t1); 
			...
			if (t1) {
				...
				if (STATUS == 1) {
					...
					*/
					...
					if (!strcmp(pd,"%%")) pd[1]=0; 
					//hc->PhysDimCode = PhysDimCode(pd); 

					ns++;
					if (ns>hdr->NS) STATUS = 99;
				}	
			}	
			t = strtok(NULL,"\xA\xD");
		}
		hdr->AS.bpb = (GDFTYP_BITS[gdftyp]*hdr->NS)>>3;
		...
		fid = fopen(fn,"r"); 
			...
			if (VERBOSE_LEVEL>7) fprintf(stdout,"cal_res: %p\n",fid); 
		...
		if (hdr->FILE.OPEN) {
			...
			for (k=0; k<hdr->NS; k++) {
				...
				hdr->CHANNEL[k].bi8 = GDFTYP_BITS[gdftyp]*k; 
			}
...
/*
	uint32_t bpb = hdr->AS.bpb; 
	if (bpb & 0x80000000)
		fprintf(stdout,"Warning SOPEN (%s) AS.bpb not defined \n",GetFileTypeString(hdr->TYPE));
		
	for (k=0, hdr->SPR = 1; k<hdr->NS; k++) {
		if ((int32_t)bpb < 0) ;
		 	// hack: CHANNEL[k].bi already defined AS.bi
		else { 
			hdr->CHANNEL[k].bi = hdr->AS.bpb;
			hdr->AS.bpb    += (hdr->CHANNEL[k].SPR * GDFTYP_BITS[hdr->CHANNEL[k].GDFTYP])>>3;
		}
		if (hdr->CHANNEL[k].SPR > 0)  // ignore sparse channels
			hdr->SPR = lcm(hdr->SPR, hdr->CHANNEL[k].SPR);
			
		if (VERBOSE_LEVEL>7) 
			fprintf(stdout,"[196]: #%i bpb=%i,spr=%i/%i\n",k,hdr->AS.bpb,hdr->SPR, hdr->CHANNEL[k].SPR);
	}
*/
	if (VERBOSE_LEVEL>7) 
		fprintf(stdout,"[197]: bpb=%i,spr=%i\n",hdr->AS.bpb,hdr->SPR);	
...
*/
		...
		uint32_t Dur[2];
	     	...
	     	if (VERBOSE_LEVEL>8) fprintf(stdout,"GDFw 010\n");
		 ...
		 */	
	     	...
	     	if (VERBOSE_LEVEL>8) fprintf(stdout,"GDFw 010 tag=%i\n",tag);
	     	tag = 2; 
 ...
 */
	...
	if (VERBOSE_LEVEL>8)
		fprintf(stdout,"sread 221 %i=?=%i  %i=?=%i \n",start,hdr->AS.first,nelem,hdr->AS.length);
...
//		else    fprintf(stderr,"              %i            %i blocks read                        (bpb=%i,pos=%li)\n",count,nelem,hdr->AS.bpb,iftell(hdr)); 
	...
	if (VERBOSE_LEVEL>7)
		fprintf(stdout,"sread 222 %i=?=%i  %i=?=%i \n",start,hdr->AS.first,count,hdr->AS.length);
...
//	hdr->data.block = (biosig_data_type*) realloc(hdr->data.block, (hdr->SPR) * count * NS * sizeof(biosig_data_type));
	...
	if (VERBOSE_LEVEL>7)
		fprintf(stdout,"sread 223  SWAP=%i spr=%i\n",SWAP,hdr->SPR);
...
;
		...
		if (hdr->TYPE==TMS32)
			...
			off = (k4+toffset)*hdr->NS*SZ + hdr->CHANNEL[k1].bi8 + k5*SZ;
	...
	if (VERBOSE_LEVEL>8)
		fprintf(stdout,"sread 225 %p off=%Li k4=%i %i %i %i bpb=%i,k1=%i k5=%i %i\n",ptr,off,k4,toffset,start, hdr->AS.first,hdr->AS.bpb,k1,k5,SZ );
;
...
size_t getNsamples(size_t p, size_t n, size_t k, HDRTYPE* hdr, biosig_data_type* buf, size_t off) {
	...
	for (i=0; i<n; ++i,++qr) {
		...
		ix = (pi-hdr->AS.rawdata_curblock)*hdr->AS.bpb + hdr->CHANNEL[k].bi+qi*sz; 
...
//			if (VERBOSE_LEVEL>8)
//				fprintf(stdout,"swrite 313= [%i %i %i %i %i ] %i %i \n",k4, k1, hdr->CHANNEL[k1].bi, k5,SZ,k4*hdr->AS.bpb + hdr->CHANNEL[k1].bi + k5*SZ,k1*nelem*hdr->SPR + k4*hdr->SPR + k5*DIV + k3);
	...
	if (VERBOSE>2) {
		...
		for (k=0; k<hdr->NS; k++) {
			...
			fprintf(fid,"\n#%2i: %3i %i %-7s\t%5f %4i %2i  %e %e %s\t%5f\t%5f\t%5f\t%5f\t%5f\t%5f\t%5f\t%5f\t%5f\t%5f",
				k+1,cp->LeadIdCode,cp->OnOff,cp->Label,cp->SPR,cp->SPR * hdr->SampleRate/hdr->SPR,
